...
type t = bool Map.M(Expression.Atom).t

let empty = Map.empty (module Expression.Atom)

let of_alist_exn l = Map.of_alist_exn (module Expression.Atom) l
...
let all expressions =
  ...
  let letters =
    ...
    |> Set.union_list (module Expression.Atom)
    |> Set.to_list
  in
  aux letters [Map.empty (module Expression.Atom)]
